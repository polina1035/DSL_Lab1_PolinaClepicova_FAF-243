...
class Grammar:
    ...
    def generate_string(self):
        ...
        # Added a safety limit of 20 iterations to prevent infinite loops in recursive rules
        iterations = 0
    ...
    def to_finite_automaton(self):
        ...
        states = self.Vn.copy()
        final_state = 'X'  # Special state for terminal productions
        ...
        for non_terminal, rules in self.P.items():
            for rule in rules:
                ...
                # If rule is A -> aB, then next state is B
                # If rule is A -> a, then next state is the sink/final state X
